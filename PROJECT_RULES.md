...
## 3. 🧩 Правила Модульности и Декомпозиции Кода (Anti-Bloat & Single Responsibility)

Разрастание файлов до сотен и тысяч строк — одна из главных причин колоссального перерасхода токенов LLM (чтение файла в 1500 строк сжигает ~10 000 токенов за один вызов), потери контекста и появления скрытых багов.

### 📏 3.1. Жесткие метрики и лимиты строк:
* **Модуль / Файл исходного кода**:
  * **Рекомендуемый целевой размер**: `150–300 строк`.
  * **Критический предел (Hard Limit)**: `400 строк`.
  * **Триггер обязательной декомпозиции**: Если при добавлении новой функции файл превышает **400 строк**, он **в этой же самой задаче обязан быть декомпозирован** на логические подмодули.
* **Функция / Метод**:
  * **Предел**: не более `40–50 строк`.
  * Если функция длиннее — разделять на чистые вспомогательные подфункции (pure helper functions) с понятным контрактом аргументов.
* **Класс**:
  * Строгое следование принципу единственной ответственности (SRP: Single Responsibility Principle). Класс обслуживает только одну бизнес-сущность или один протокол взаимодействия.

---

### 📂 3.2. Стандарты декомпозиции по слоям системы:

#### 🌐 1. FastAPI Роутеры (`app/api/`):
* **Эталон декомпозиции в проекте**: подсистема маркировки Честного Знака [`app/api/kiz/`](file:///d:/PyCharm_Projects/WB%20FBS/app/api/kiz/). Вместо единого 1500-строчного файла она разбита на 5 специализированных подмодулей:
  * [`attach.py`](file:///d:/PyCharm_Projects/WB%20FBS/app/api/kiz/attach.py) — привязка, отвязка, валидация КИЗ (~140 строк).
  * [`documents.py`](file:///d:/PyCharm_Projects/WB%20FBS/app/api/kiz/documents.py) — подготовка и отправка документов вывода/возврата (~180 строк).
  * [`signature_batches.py`](file:///d:/PyCharm_Projects/WB%20FBS/app/api/kiz/signature_batches.py) — пакетная очередь на подпись УКЭП (~220 строк).
  * [`archive.py`](file:///d:/PyCharm_Projects/WB%20FBS/app/api/kiz/archive.py) — парсинг и сверка выгрузок архива WB (~190 строк).
  * [`auto_queue.py`](file:///d:/PyCharm_Projects/WB%20FBS/app/api/kiz/auto_queue.py) — управление автоочередью маркировки (~130 строк).
* **Правило**: Если роутер в `app/api/<feature>.py` разрастается свыше 300 строк или объединяет >2 различных поддомена, его необходимо преобразовать в пакет `app/api/<feature>/` с агрегацией под-роутеров в `__init__.py`.

#### ⚙️ 2. Сервисный слой (`app/services/`):
* **Разделение I/O клиентов и бизнес-логики**:
  * `*_client.py` (например, `wb_client.py`, `cz_client.py`) — отвечают **только** за HTTP-транспорт, авторизацию, таймауты, ретраи и сетевые ошибки.
  * `*_service.py` (например, `kiz_service.py`, `auto_kiz_queue_service.py`, `archive_service.py`) — отвечают **только** за валидацию, парсинг, трансформацию и бизнес-правила.
* **Утилиты**: выносить чистые хелперы в компактные модули (`time_service.py`, `encryption.py`), категорически избегая создания "мусорных" файлов `utils.py` / `helpers.py` на 1000 строк.

#### 🤖 3. Агенты Celery (`app/agents/`):
* **Один агент — один файл**: каждый фоновый агент живет в собственном компактном файле (`order_poller.py`, `supply_agent.py`, `cz_withdrawal.py`, `morning_digest.py` и т.д.).
* **Таска Celery — это ТОЛЬКО координатор**:
  * ❌ **Запрещено**: писать 150 строк логики прямо внутри тела функции `@celery_app.task`.
  * ✅ **Правильно**: таска получает параметры → открывает сессию БД → вызывает методы соответствующих сервисов из `app/services/` → фиксирует статус в `AuditLog` → инициирует downstream-цепочку или уведомление.

#### 🗄️ 4. Модели БД (`app/models/`) и Схемы Pydantic (`app/schemas/`):
* Каждая предметная область хранится в отдельном файле (`order.py`, `seller.py`, `kiz.py`, `supply.py`, `user.py`, `audit.py`).
* Запрещено объединять несвязанные сущности в один файл ради «экономии файлов».

#### 🧪 5. Набор тестов (`tests/`):
* Тесты разбиваются строго по фичам и сценариям: `tests/test_<feature_or_milestone>.py`.
* Запрещено создавать монолитные файлы тестов на тысячи строк. Если сьют тестов для домена растет, разделять его на `test_<feature>_unit.py`, `test_<feature>_adversarial.py` или `test_<feature>_integration.py`.

#### 💻 6. Модульность Фронтенда (`frontend/js/`):
* **Разделение по доменам и слоям ответственности**:
  * `state.js` — глобальные переменные состояния, кэш, словари и константы (`STATUS_MAP`, `STATUS_MAP_NK`).
  * `api.js` — унифицированный HTTP-клиент (`apiFetch`, обработка JWT, 401 redirect, toast-ошибки).
  * `ui.js` — базовые UI-компоненты (тосты, модальные окна, переключение табов и светлой/темной темы).
  * `<domain>.js` (`orders.js`, `supplies.js`, `kiz.js`, `sellers.js`, `national_catalog.js`, `signature_queue.js`, `dashboard.js`, `audit.js`) — изолированная логика конкретных страниц.
* **Правило**: Запрещено помещать логику новой страницы или функционала в `app.js` или `ui.js`. Новый домен оформляется в свой собственный `<domain>.js` файл и подключается в `index.html`.

---

## 4. 📋 Сквозной Чеклист Доработки Функционала (Holistic Protocol)
...
- [ ] **3. Лимит строк и декомпозиция (Anti-Bloat Limit)**:  
  Проверить размер изменяемых файлов. Если файл превышает 400 строк — декомпозировать на подмодули. Функции держать в пределах 40–50 строк.
- [ ] **4. Сохранение данных и неразрушающие миграции**:  
  Изменения схемы БД должны быть обратно совместимы с существующими данными. Запрещено удалять колонки без миграции данных.
- [ ] **5. Регистрация задач и очередей Celery**:  
...
- [ ] **6. Обязательный аудит-след (`AuditLog`)**:  
...
- [ ] **7. Написание автотестов (`tests/test_<feature>.py`)**:  
...
- [ ] **11. Инструкция по обновлению VPS в ответе пользователю**:  
  В финальном ответе всегда выдать точные команды деплоя на сервере.

---

## 5. 🛡️ Ограничения Окружения и Production Guardrails (VPS 1 GB RAM / 2 GB Swap)
...
## 6. 🔑 Безопасность, Авторизация и Самоисцеление (Self-Healing Auth)
...
## 7. 🏷️ Маркировка Честный Знак и True API Инварианты
...
## 8. ❌ Анти-Паттерны (Чего делать КАТЕГОРИЧЕСКИ нельзя)
...
| Создавать файлы-монолиты (>400 строк) | Сжигает до 12 000 токенов на просмотр, падает точность | Декомпозировать на подпакеты и узкие сервисы (Раздел 3) |
...
## 9. 💻 Памятка Команд (Developer & Ops Cheatsheet)
